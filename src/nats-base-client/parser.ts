      ...
      switch (this.state) {
        case State.OP_START:
          switch (b) {
            ...
            case  "9".charCodeAt(0):
              // @ts-ignore
              let a = buf.toLocaleString().split(',').map(x => String.fromCharCode(x)).join('')
              bodyLength = parseInt(String.fromCharCode(buf[i]) + String.fromCharCode(buf[i+1]));
              const data = buf.subarray(i + 2, i + bodyLength + 2);
              this.ma = newMsgArg();
              this.ma.sid = 1;
              this.ma.hdr = 0;
              this.ma.size = bodyLength;
              this.dispatcher.push(
                  { kind: Kind.MSG, msg: this.ma, data: data },
              );
              this.argBuf = undefined;
              // buf = buf.subarray(0,this.msgBuf.length);
            ...
            default:
              ...
              // this.ma = newMsgArg();
              // this.ma.sid = 1
              // let sepIndex = buf.findIndex(x => x === 13);
              //
              // this.msgBuf = new DenoBuffer();
              // this.msgBuf._buf = buf.slice(0,sepIndex)
              // this.ma.size = this.msgBuf._buf.length
              // i--
              break;
